			...
			<div class="row">
				<form class="needs-validation col-8 col-sm-9 col-md-8 col-lg-6 col-xl-6 mx-auto" action="index.php?p=modifier-recette" method="POST" novalidate>
					...
					<div class="mb-5">
						...
						<input type="text" class="form-control" name="title" id="recipe-update-title" value="<?= htmlspecialchars($this->recipe['title'], ENT_COMPAT) ?>" required>
					...
					<div class="md-5 mb-5">
						...
						<input type="text" class="form-control" name="difficulty" id="recipe-update-difficulty" value="<?= htmlspecialchars($this->recipe['difficulty'], ENT_COMPAT) ?>" required>
						<div class="invalid-feedback">Niveau de difficulté manquant.</div>
					</div>
					<div class="md-5 mb-5">
						<label for="recipe-update-cost" class="form-label">Coût:</label>
						<input type="text" class="form-control" name="cost" id="recipe-update-cost" value="<?= htmlspecialchars($this->recipe['cost'], ENT_COMPAT) ?>" required>
						<div class="invalid-feedback">Coût manquant.</div>
					</div>
					<div class="mb-5">
						<label for="recipe-update-description" class="form-label">Description:</label>
						<textarea name="description" class="form-control" id="recipe-update-description" rows="5" required><?= htmlspecialchars($this->recipe['description'], ENT_COMPAT) ?></textarea>
					...
					<div class="mb-5">
						...
						<textarea name="ingredients" class="form-control" rows="10" id="recipe-update-ingredients" required><?= htmlspecialchars($this->recipe['ingredients'], ENT_COMPAT) ?></textarea>
						<div class="invalid-feedback">Ingrédients manquants.</div>
					</div>
					<div class="mb-5">
						<label for="recipe-update-steps" class="form-label">Etapes:</label>
						<textarea name="steps" class="form-control" rows="10" id="recipe-update-steps" required><?= htmlspecialchars($this->recipe['steps'], ENT_COMPAT) ?></textarea>
